...
public record CompositeRenderTypeData(VertexFormat format, VertexFormat.Mode mode, int bufferSize,
    ...
    private static final Codec<List<List<RenderTypeLayer>>> LAYER_CODEC = CodecUtil.withAlternative(
            ...
            RenderTypeLayer.CODEC.listOf().flatXmap(
                    ...
                    list -> list.size() == 1 ? DataResult.success(list.get(0)) : DataResult.error(() -> "Expected 1 shard")
            )).flatXmap(
